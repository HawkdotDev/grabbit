...
export const TerminalDrawer: React.FC<TerminalDrawerProps> = ({ downloads, globalSpeed }) => {
  ...
  return (
    <footer className="bg-[#1e1e1e] border-t border-[#2e2e2e] flex flex-col font-mono text-xs select-none shrink-0">
      {/* Drawer Header Tabs */}
      <div className="h-9 px-4 flex items-center justify-between bg-[#141414] border-b border-[#292929]">
        <div className="flex items-center gap-4 font-sans font-semibold">
          <button
            ...
            className={`flex items-center gap-1.5 py-1 text-xs cursor-pointer ${
              activeTab === 'problems'
                ? 'text-[#e44232] font-bold border-b-2 border-[#e44232]'
                : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            <span>PROBLEMS</span>
            <span className="px-1.5 py-0.2 bg-white/5 rounded-full text-[10px] text-slate-300 font-mono">
              {errorCount}
            </span>
          </button>
      ...
      {!isCollapsed && (
        <div className="h-28 bg-[#141414] p-3 overflow-y-auto space-y-1 text-[11px] text-slate-300 font-mono">
          ...
          {activeDownloads.map((d) => (
            <div key={d.id} className="flex items-center gap-2">
              <span className="text-[#e44232] font-bold">[ENGINE ACTIVE]</span>
      ...
      <div className="h-7 px-4 bg-[#0e0e0e] border-t border-[#292929] flex items-center justify-between text-[11px] text-slate-400 font-mono">
        <div className="flex items-center gap-4">
          <span className="flex items-center gap-1 text-slate-300">
            <span className="h-2 w-2 rounded-full bg-[#e44232]" />
        ...
        <div className="flex items-center gap-4 font-sans">
          ...
          <span>UTF-8</span>
          <span>CRLF</span>
          <span className="text-[#e44232] font-semibold">TypeScript</span>
        </div>
      </div>
    </footer>
  )
