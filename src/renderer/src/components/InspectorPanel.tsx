...
export const InspectorPanel: React.FC<InspectorPanelProps> = ({
  ...
  return (
    <aside className="w-80 bg-[#18191d] border-l border-[#2a2d34] p-4 flex flex-col gap-4 font-mono text-xs select-none shrink-0">
      {/* Inspector Header */}
      <div className="bg-[#121316] p-3 rounded-xl border border-[#2a2d34] flex items-center justify-between">
        <div className="flex items-center gap-2">
          <RefreshCw className="h-4 w-4 text-[#a3e635] animate-spin" />
          <span className="font-bold text-slate-200">Tasks Evaluated</span>
        </div>
        <span className="px-2 py-0.5 rounded-full bg-[#202228] text-[#a3e635] font-bold">
          {downloads.length}
        </span>
      </div>

      {/* Issues / Error List Card */}
      <div className="bg-[#121316] p-3 rounded-xl border border-[#2a2d34] space-y-2">
      ...
      <div className="bg-[#121316] p-3 rounded-xl border border-[#2a2d34] space-y-2">
        <div className="flex items-center justify-between text-slate-300 font-bold">
          ...
          <span className="text-[#a3e635]">32 Threads Ready</span>
        </div>
        ...
        <div className="space-y-1.5 pt-1 text-[11px] text-slate-400">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-[#a3e635] font-bold">ENABLED</span>
          </div>
        </div>
      </div>
      ...
      <div className="mt-auto space-y-2">
        <button
          onClick={onResumeAll}
          className="w-full py-3 px-4 bg-[#ccff00] hover:bg-[#b8e600] active:scale-95 text-slate-950 rounded-xl font-bold text-xs flex items-center justify-center gap-2 shadow-lg shadow-[#ccff00]/10 transition duration-150 cursor-pointer font-sans"
        >
        ...
        {completedCount > 0 && (
          <button
            ...
            className="w-full py-2 px-3 bg-[#202228] hover:bg-[#282b33] text-slate-300 rounded-xl border border-[#2a2d34] font-semibold text-xs flex items-center justify-center gap-2 transition cursor-pointer font-sans"
          >
